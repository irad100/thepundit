...
class Stdout(BaseWriter):
    ...
    def live_scores(self, live_scores):
        ...
        for league, games in groupby(scores, key=lambda x: x["league"]):
            ...
            for game in games:
                ...
                print(colors[self.colors.TIME] + '   %s' % Stdout.utc_to_local(game["time"], use_12_hour_format=False) + colors['end'])
                print()
    ...
    def team_scores(self, team_scores, time, show_datetime, use_12_hour_format):
        ...
        for score in team_scores["matches"]:
            if score["status"] == "FINISHED":
                print(colors[self.colors.TIME] + "%s\t" % score["utcDate"].split('T')[0] + colors['end'], end="")
                self.scores(self.parse_result(score))
            elif show_datetime:
                self.scores(self.parse_result(score), add_new_line=False)
                print(colors[self.colors.TIME] + '   %s' % Stdout.utc_to_local(score["utcDate"], use_12_hour_format, show_datetime) + colors['end'])
    ...
    def standings(self, league_table, league):
        ...
        for team in league_table["standings"][0]["table"]:
            ...
            if cl_upper <= team["position"] <= cl_lower:
                ...
                print(colors[self.colors.EL_POSITION] + team_str + colors['end'])
            elif rl_upper <= team["position"] <= rl_lower:
                print(colors[self.colors.RL_POSITION] + team_str + colors['end'])
            else:
                print(colors[self.colors.POSITION] + team_str + colors['end'])
    ...
    def league_scores(self, total_data, time, show_datetime,
        ...
        for match in total_data['matches']:
            ...
            if show_datetime:
                print(colors[self.colors.TIME] + '   %s' % Stdout.utc_to_local(match["utcDate"], use_12_hour_format, show_datetime) + colors['end'])
            print()

    def league_header(self, league):
        """Prints the league header"""
        league_name = " {0} ".format(league)
        print(colors[self.colors.MISC] + "{:=^62}".format(league_name) + colors['end'])
        print()
    ...
    def scores(self, result, add_new_line=True):
        ...
        print(colors[homeColor] + '%-25s %2s' % (result.homeTeam, result.goalsHomeTeam) + colors['end'], end="")
        print("  vs ", end="")
        print(colors[awayColor] + '%2s %s' % (result.goalsAwayTeam, result.awayTeam.rjust(25)) + colors['end'], end="\n\n")
...
class Csv(BaseWriter):
    ...
    def generate_output(self, result):
        if not self.output_filename:
            for row in result:
                print(u','.join(unicode(item) for item in row))
        else:
    ...
    def generate_output(self, result):
        if not self.output_filename:
            ...
            with io.open(self.output_filename, 'w', encoding='utf-8') as f:
                data = json.dumps(result, f, indent=4, separators=(',', ': '), ensure_ascii=False)
                f.write(data)
